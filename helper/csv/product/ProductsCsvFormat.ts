...
/**
 * Canonical header order for full product import/export files.
 */
...
/**
 * Column metadata used by the product CSV editor/import wizard.
 *
 * Each column can define field type, tooltips, required flags, default values,
 * and variant/product-only behavior.
 */
...
/**
 * Validation helpers for full product CSV files.
 */
export class ProductsCsvFormat {
  ...
  /**
   * Checks whether the uploaded product CSV contains all required headers.
   * @param {string[]} headers - Parsed CSV headers.
   * @returns {boolean} True when all headers from `ProductsCsvHeaders` exist.
   */
